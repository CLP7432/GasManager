...
@Entity
@Table(name = "aceites_dispensario")
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class AceiteDispensario {
    ...
    private Integer stockMaximo = 20;
    ...
    public void descontarStock(Integer cantidad) {
        if (!isStockDisponible(cantidad)) {
            throw new IllegalStateException("Stock insuficiente en dispensario: " + nombre);
        }
        this.stockActual -= cantidad;
    }

    public void aumentarStock(Integer cantidad) {
        if (stockMaximo != null && (stockActual + cantidad) > stockMaximo) {
            throw new IllegalStateException("No se puede aumentar stock, excede la capacidad máxima: " + stockMaximo);
        }
        this.stockActual += cantidad;
    }
}